...
public class AdminServiceImplTest {
    ...
    public void should_return_show_details_when_invoke_view_show_number_if_record_found () {
        when(bookingRepository.findPhoneNoByTicketIdIn(any(List.class))).thenReturn(Arrays.asList("90091203", "90082713"));
        when(showRepository.findById(any())).thenReturn(Optional.of(Show.builder()
                .seatReserved(Arrays.asList("C10", "D7", "E9"))
                .ticketIdAllocated(Arrays.asList("001", "002"))
                .build()));
